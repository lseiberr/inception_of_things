...
if ! command -v k3d >/dev/null 2>&1; then
  echo "[k3d] Installation de k3d…"
  # Télécharger le script d'install localement
  if curl -fsSL -o /tmp/k3d-install.sh https://raw.githubusercontent.com/k3d-io/k3d/main/install.sh; then
    chmod +x /tmp/k3d-install.sh
    /tmp/k3d-install.sh
  else
    echo "[k3d] WARNING: impossible de télécharger le script d'installation (GitHub 503 ?), on continue sans k3d."
  fi
fi
